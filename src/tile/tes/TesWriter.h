...
private:
	...
	{
	public:
		explicit SortedFeature(TFeature* feature) :
	...
	using CompareFunc = bool(*)(const TSharedElement*, const TSharedElement*);

	template <typename T>
	void gatherSharedItems(const ElementDeduplicator<T>& items, int minUsers, 
		size_t firstGroupSize, CompareFunc compare = nullptr);
	...
	void writeStringValue(TElement::Handle strHandle);
	void writeRelationTable(const TRelationTable* relTable);
	void writeFeatures();
	void writeNode(TNode* node);
	void writeWay(TWay* way);
	void writeRelation(TRelation* relation);
