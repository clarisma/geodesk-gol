...
void ChangeModelDumper::dumpFlags(ChangeFlags flags)
{
    ...
    if (test(flags, ChangeFlags::FLAGS_CHANGED)) tempStrings_.push_back("flags_changed");
    if (test(flags, ChangeFlags::FLAGGED_SHARED_LOCATION)) tempStrings_.push_back("flagged_shared_location");
    if (test(flags, ChangeFlags::FLAGGED_EXCEPTION_NODE)) tempStrings_.push_back("flagged_exception_node");
    ...
    if (test(flags, ChangeFlags::WAY_WILL_HAVE_FEATURE_NODES)) tempStrings_.push_back("way_will_have_feature_nodes");
