...
{
public:
	explicit TileIndexBuilder(const BuildSettings& settings);
	void build(NodeCountTable nodeCounts);
	int tileCount() const { return tileCount_; }
	ZoomLevels zoomLevels() const { return settings_.zoomLevels(); }
	...
	std::unique_ptr<const Tip[]> takePileToTip() { return std::move(pileToTip_); }
	std::unique_ptr<const uint32_t[]> takeTileSizeEstimates() { return std::move(tileSizeEstimates_); }

	static constexpr int ESTIMATED_BYTES_PER_NODE = 10;
	static constexpr int ESTIMATED_CHILD_BYTES_PER_PARENT_BYTE = (1 << 14);
...
private:
	struct Tier
	{
		...
		{
		public:
			explicit Iterator(STile* first) : next_(first) {}
			STile* next() noexcept;
	...
	constexpr int cellOf(int col, int row) const
	{
	...
	void createLeafTiles(NodeCountTable nodeCounts);
	void addParentTiles();
	void trimTiles();
	void linkChildTiles();
	...
	static Tip assignTip(STile* tile, Tip tip) noexcept;
