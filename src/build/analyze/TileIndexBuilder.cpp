...
void TileIndexBuilder::build(NodeCountTable nodeCounts)
{
	createLeafTiles(std::move(nodeCounts));
	addParentTiles();
	trimTiles();
	linkChildTiles();
	...
	pileToTip_.reset(new Tip[tileCount_ + 1]);
...
void TileIndexBuilder::createLeafTiles(NodeCountTable nodeCounts)
{
	int extent = 1 << maxZoom_;
	const uint32_t* p = nodeCounts.data();
	STile* tile = nullptr;
	uint32_t count = 0;
	...
	Tile tile, int maxChildCount, uint64_t nodeCount, STile* next)
{
	...
	pt->estimatedTileSize = nodeCount * ESTIMATED_BYTES_PER_NODE + 32;
		// add some minimum value so estimated tile size is never zero
	return pt;
}
...
Tip TileIndexBuilder::assignTip(STile* tile, Tip tip) noexcept
{
	tile->tip = tip;
	Tip childTip (tip + ((tile->maxChildCount == 64) ? 3 : 2));
	Tip nextTip (childTip + tile->childCount);
