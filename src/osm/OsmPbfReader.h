...
{
public:
	explicit OsmPbfContext(void* reader) :
		reader_(static_cast<Reader*>(reader)),
...
protected:
	...
	*/
	...
	void stringTable(ByteSpan strings)  // CRTP override
	{
...
private:
	...
	void decodePrimitiveGroup(ByteSpan group)
	{
		const uint8_t* p = group.data();
		while (p < group.end())
		{
			uint32_t field = readVarint32(p);
			switch (field)
			...
			case GROUP_DENSENODES:
				self()->beginNodeGroup();		// TODO: check!!!!
				decodeDenseNodes(protobuf::readMessage(p));
				self()->endNodeGroup();			// TODO: check!!!!
				break;
			case GROUP_WAY:
				self()->beginWayGroup();		// TODO: check!!!!
				decodeWay(protobuf::readMessage(p));
				self()->endWayGroup();			// TODO: check!!!!
				break;
			case GROUP_RELATION:
				self()->beginRelationGroup();	// TODO: check!!!!
				decodeRelation(protobuf::readMessage(p));
				self()->endRelationGroup();		// TODO: check!!!!
				break;
	...
	void decodeDenseNodes(ByteSpan data)
	{
		...
		if (ids.data())
		{
			...
			const uint8_t* pId = ids.data();
			const uint8_t* pLat = lats.data();
			const uint8_t* pLon = lons.data();
			int64_t id = 0;
			int64_t lat = 0;
			int64_t lon = 0;
			...
			while (pId < ids.end())
			{
				...
				const uint8_t* pNextTags = self()->node(id,
					static_cast<int32_t>(lonInNanoDeg / 100),
					static_cast<int32_t>(latInNanoDeg / 100), tags);
				tags = ByteSpan(pNextTags, tags.end());
			}
			assert(pId == ids.end());
	...
	void decodeRelation(ByteSpan data)
	{
		int64_t id = 0;
		ByteSpan keys;
		ByteSpan values;
...
{
public:
	explicit OsmPbfReader(int numberOfThreads) :
...
private:
	...
	{
		ReusableBlock headerBlock(64);
		uncompressBlock(block, headerBlock);
		const uint8_t* p = headerBlock.data();
		const uint8_t* pEnd = p + headerBlock.size();
		while(p < pEnd)
		{
			uint32_t field = readVarint32(p);
			switch (field)
			...
			case HEADER_WRITINGPROGRAM:
				metadata_.generator = readStringView(p);
				break;
			case HEADER_SOURCE:
				metadata_.source = readStringView(p);
				break;
			case HEADER_REPLICATION_TIMESTAMP:
				metadata_.replicationTimestamp = DateTime(readVarint64(p) * 1000);
				break;
			case HEADER_REPLICATION_SEQUENCE:
				metadata_.replicationSequence = readVarint32(p);
				break;
			case HEADER_REPLICATION_URL:
				metadata_.replicationUrl = readStringView(p);
				break;
			default:
				protobuf::skipEntity(p, field);
				break;
